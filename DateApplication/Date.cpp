...
//Note: Must be done outside the class declaration
...
//A static function that returns to true if the given year is leap year
bool Date::isLeapYear(int year){
	return ((year % 4 == 0 && year % 100 != 0) || (year % 400 == 0));
}

//Validation for valid date y, m, d
...
//Wiki "Determination of the day of the week" algorithm
...
int Date::getDayOfWeek(int y, int m, int d){
	...
	//Date before 17xx are not valid but needed validation to prevent negative index
...
void Date::setDate(int y, int m, int d){
	setYear(y);
	setMonth(m);
	setDay(d);	//need to set day after year and month
}
...
//Assuming year and month has already been set
...
//Increment this instance to the next day, return this instance by reference
Date& Date::nextDay(){
	int lastDayOfMonth = DAYS_IN_MONTHS[month-1];
	if(month == 2 && isLeapYear(year)){
		lastDayOfMonth = 29;
	...
	//check day against the day of the month
...
//Decrement this instance to previous day, return this instance by reference
Date& Date::previousDay(){
	...
	//check day against the end of the month
	if(--day < 1){
		day = lastDayOfMonth;
		if(--month < 1){
			month = 12;
...
//Increment this instance to next month, return this instance by reference
Date& Date::nextMonth(){
	if(++month > 12){
		month = 1;
		if(++year > YEAR_MAX){
	...
	//We have to adjust the last day of the month
	int lastDayOfMonth = DAYS_IN_MONTHS[month-1];
	if(month == 2 && isLeapYear(year)){
		lastDayOfMonth = 29;
	}
...
//Decrement this instance to previous month, return this instance by reference
...
//Increment this instance to next year, return this instance by reference
Date& Date::nextYear(){
	if(++year > YEAR_MAX){
		throw out_of_range("Error: Next year is out of range!");
	}

	//We have to adjust last day of the month, leap year to non-leap year
...
//Decrement this instance to previous year, return this instance by reference
